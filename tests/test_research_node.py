...
def test_research_agent_combines_and_dedupes_by_domain():
    ...
    fake_article = {
        "resolved_url": "https://outleta.com/a1",
        "title": "News Article 1",
        "text": "Full article body text here.",
        "published_date": "2026-09-14",
        "image_url": "https://outleta.com/img.jpg",
        "error": None,
    }
    ...
    with (
        ...
        patch("backend.graph.research_node.fetch_article", return_value=fake_article),
    ):
    ...
    assert "https://outleta.com/a1" in domains
    assert "https://outletb.com/a2" in domains
    assert "https://outletc.com/w2" in domains
...
def test_research_agent_only_fetches_full_text_for_top_n():
    ...
    fake_article = {
        "resolved_url": "https://fetched.com",
        "title": "Fetched Title",
        "text": "Full text.",
        "published_date": "2026-09-14",
        "image_url": None,
    ...
    with (
        ...
        patch("backend.graph.research_node.fetch_article", return_value=fake_article) as mock_fetch,
    ):
...
def test_research_agent_builds_labeled_source_material():
    ...
    fake_article = {
        "resolved_url": "https://outlet.com/a1",
        "title": "First Article",
        "text": "Detailed body content.",
        "published_date": "2026-09-14",
        "image_url": None,
        "error": None,
    }
    ...
    assert "Detailed body content." in result["source_material"]
...
def test_research_agent_respects_max_sources_cap():
    ...
    with (
        ...
        patch("backend.graph.research_node.search_web", return_value=[]),
        patch(
            "backend.graph.research_node.fetch_article",
            return_value={
                "resolved_url": "x",
                "title": "x",
                "text": "x",
